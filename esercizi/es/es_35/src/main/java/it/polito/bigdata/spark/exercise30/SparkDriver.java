...
public class SparkDriver {
	...
	public static void main(String[] args) {
		...
		List<Float> topList = valuesRDD.top(1);
		Float top = topList.get(0);
		JavaRDD<String> topLine = logRDD.filter(e -> e.contains(top.toString()));

		JavaRDD<String> topDate = topLine.map( e -> {
			String[] array = e.split(",");
			return array[1];
		});
