...
class Mapper2BigData extends Mapper<
    ...
    protected void map(
        ...
        if (inputUsers.contains(users[0]) && !users[1].equalsIgnoreCase("user2") && !output.contains(users[1])) {
            context.write(NullWritable.get(), new Text(users[1]));
            output.add(users[1]);
        }
        if (inputUsers.contains(users[1]) && !users[0].equalsIgnoreCase("user2") && !output.contains(users[0])) {
    ...
    protected void cleanup(Mapper<LongWritable, Text, NullWritable, Text>.Context context) throws IOException, InterruptedException {
        System.out.println("Siamo nel cleanup");
